        ...
        try:
            ...
            if not destination_path:
                return PublishWorkflowResultFailure(
                    ...
                    exception=ValueError("Missing 'publish_output_directory' on Start Flow node."),
                )
            ...
            # Copy libraries
            self._emit_progress_event(additional_progress=17.0, message="Copying libraries...")
            ...
            # Copy static files referenced by nodes (e.g. SelectFromProject)
            self._emit_progress_event(additional_progress=3.0, message="Copying static files...")
            all_nodes = self._collect_all_nodes_from_workflow()
            file_param_values = self._gather_static_file_references(all_nodes)
            if file_param_values:
                self._copy_static_files(file_param_values, destination)
...
"""
    ...
    # -- Static file gathering and copying --

    def _collect_all_nodes_from_workflow(self) -> list[BaseNode]:
        ...
        logger.info("Collected %d nodes from workflow.", len(nodes))
        for node in nodes:
            logger.info("  Node: '%s' (%s)", node.name, type(node).__name__)
        return nodes

    @staticmethod
    def _gather_static_file_references(nodes: list[BaseNode]) -> list[tuple[str, str]]:
        """Scan all nodes for file references that may point to static assets.

        Detects SelectFromProject nodes (input file paths via selected_path parameter)
        by checking node metadata.
        ...
        logger.info("Gathered %d static file references.", len(results))
        return results

    @staticmethod
    def _resolve_file_reference(value_str: str, project_root: Path | None) -> tuple[Path, Path] | None:
        """Resolve a file reference string to an absolute path and project-relative path.

        Tries macro resolution first, then falls back to treating the value as an
        absolute path relative to the project root.

        Returns:
            Tuple of (absolute_path, relative_path) or None if resolution fails.
        """
        # Try macro resolution first
        try:
            ...
            logger.debug("Macro parse/resolve failed for '%s'.", value_str)

        # Fall back to treating value as an absolute path
    ...
    def _copy_static_files(
        ...
        Handles two forms of value strings:
        - Macro paths (e.g. "{inputs}/image.png") — resolved via GetPathForMacroRequest
        - Absolute paths (e.g. "/Users/.../outputs/file.png") — resolved via the project system
        """
        copied: set[Path] = set()

        # Get the project root to compute relative paths for absolute path values
        ...
        for node_name, value_str in file_param_values:
            ...
            if resolved is None:
                logger.debug("Could not resolve '%s' from node '%s', skipping.", value_str, node_name)
                continue

            absolute_path, resolved_relative = resolved

            if not absolute_path.exists():
                logger.debug("Resolved path does not exist: %s (node '%s'), skipping.", absolute_path, node_name)
                continue
            ...
            if absolute_path.is_dir():
                ...
                self._copy_file(absolute_path, dest)
