...
`test_griptape_cloud_headers.py` polices the requests this library builds itself, and cannot see
these: the driver builds `Authorization` inside the framework, so the factory's dict never
reaches the wire unless `cloud_driver_auth` puts it there.
"""
...
# inside `build_tool_from_config`. Each exempt function must still reach the helper; see below.
...
def _cloud_driver_constructions() -> dict[tuple[Path, str], list[tuple[int, bool]]]:
    ...
    for path in sorted(LIBRARY_ROOT.rglob("*.py")):
        ...
        # Sorted: `ast.walk` is breadth-first, so a failure would name lines out of file order.
...
def test_file_manager_driver_still_rejects_the_headers_kwarg() -> None:
    ...
    `headers` is `init=False` here but accepted by the other two drivers. If `griptape` settles
    that, this fails -- the signal to drop the workaround.
    """
...
def test_every_cloud_driver_construction_carries_attribution() -> None:
    ...
    Nothing warns: the platform emits no degradation metric for a *missing* attribution header.
    """
    unspread = {
...
def test_every_exemption_still_reaches_the_helper() -> None:
    ...
    Scoped to the exempt function, not its file: a sibling's use says nothing about this one.
    """
...
def test_exemptions_cover_exactly_the_constructions_they_were_written_for() -> None:
    ...
    The second half bites: a driver added to an already-exempt function inherits its pass and
    falls back to the empty `GT_CLOUD_API_KEY` the engine plants -- a 401, unattributed.
    """
...
def test_driver_credentials_are_not_serialized() -> None:
    ...
    Neither field carries `serializable` metadata, so `Agent.from_dict` alone rebuilds a driver
    from `os.environ` with no attribution. `_restored_cloud_credentials` repairs at load instead,
    which is the right layer: marking either field serializable would write the raw credential
    into every saved workflow on disk. If this fails, upstream changed its mind about that and
    the security of every saved workflow needs re-examining before the injection is dropped.
    """
