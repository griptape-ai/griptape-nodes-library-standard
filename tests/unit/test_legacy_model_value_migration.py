...
def _model_access_component(node: BaseNode) -> ModelAccessComponent:
    """Resolve the node's `ModelAccessComponent`.

    Every node that installs a model-access dropdown assigns the
    `ModelAccessComponent` to `node._model_access` directly. Cast to `Any`
    since `_model_access` isn't part of `BaseNode`'s declared surface, and its
    type differs across the node's many possible base classes.
    """
    return cast("Any", node)._model_access
