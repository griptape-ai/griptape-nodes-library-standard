...
# dependencies = []
# [tool.griptape-nodes]
# name = "test_cartwheel_motion_generation"
...
# node_types_used = [["Griptape Nodes Testing Library", "AssertString"], ["Griptape Nodes Library", "EndFlow"], ["Griptape Nodes Library", "CartwheelMotionGeneration"], ["Griptape Nodes Library", "StartFlow"], ["Griptape Nodes Library", "ToText"]]
...
with GriptapeNodes.ContextManager().flow(flow_name):
    gen_node = GriptapeNodes.handle_request(
        CreateNodeRequest(
            node_type="CartwheelMotionGeneration",
            specific_library_name="Griptape Nodes Library",
            node_name="CartwheelMotionGeneration",
            metadata={},
            resolution="resolved",
            initial_setup=True,
        )
    ...
    with GriptapeNodes.ContextManager().node(start_node):
        ...
        GriptapeNodes.handle_request(
            AddParameterToNodeRequest(
                ...
                input_types=["any"],
                output_type="str",
                ui_options={"display_name": "Character ID"},
                parent_container_name="",
                initial_setup=True,
            )
        )
    ...
    GriptapeNodes.handle_request(
        CreateConnectionRequest(
            source_node_name=start_node,
            source_parameter_name="character_id",
            target_node_name=gen_node,
            target_parameter_name="character_id",
            initial_setup=True,
        )
    )
    GriptapeNodes.handle_request(
...
if __name__ == "__main__":
    ...
    if args.json_input is None:
        ...
        if args.character_id is not None:
            flow_input["Start Flow"]["character_id"] = args.character_id
