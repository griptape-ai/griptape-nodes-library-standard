...
# dependencies = []
# [tool.griptape-nodes]
# name = "test_seedance_v2_first_last_frame"
...
# node_types_used = [["Griptape Nodes Testing Library", "AssertFileExists"], ["Griptape Nodes Library", "CreateColorBars"], ["Griptape Nodes Library", "EndFlow"], ["Griptape Nodes Library", "SeedanceVideoGeneration"], ["Griptape Nodes Library", "ToText"]]
...
with GriptapeNodes.ContextManager().flow(flow_name):
    first_frame_source = GriptapeNodes.handle_request(
        CreateNodeRequest(
            node_type="CreateColorBars",
            specific_library_name="Griptape Nodes Library",
            node_name="First Frame Source",
            metadata={},
            resolution="resolved",
            initial_setup=True,
        )
    ).node_name
    last_frame_source = GriptapeNodes.handle_request(
        CreateNodeRequest(
            node_type="CreateColorBars",
            specific_library_name="Griptape Nodes Library",
            node_name="Last Frame Source",
            metadata={},
            resolution="resolved",
            initial_setup=True,
        )
    ...
    with GriptapeNodes.ContextManager().node(gen_node):
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                ...
                node_name=gen_node,
                value="Seedance 2.0 First Last Frame",
                initial_setup=True,
                is_output=False,
            )
        )
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                ...
                node_name=gen_node,
                value="Transition between these two frames",
                initial_setup=True,
                is_output=False,
            )
        )
    GriptapeNodes.handle_request(
        CreateConnectionRequest(
            source_node_name=first_frame_source,
    ...
    GriptapeNodes.handle_request(
        CreateConnectionRequest(
            source_node_name=last_frame_source,
            source_parameter_name="image",
            target_node_name=gen_node,
            target_parameter_name="last_frame",
            initial_setup=True,
        )
    )
    GriptapeNodes.handle_request(
