...
# dependencies = []
# [tool.griptape-nodes]
# name = "test_omnihuman_video_generation"
...
# node_types_used = [["Griptape Nodes Testing Library", "AssertFileExists"], ["Griptape Nodes Library", "ElevenLabsTextToSpeechGeneration"], ["Griptape Nodes Library", "EndFlow"], ["Griptape Nodes Library", "GenerateImage"], ["Griptape Nodes Library", "OmnihumanVideoGeneration"], ["Griptape Nodes Library", "ToText"]]
...
with GriptapeNodes.ContextManager().flow(flow_name):
    ...
    with GriptapeNodes.ContextManager().node(source_node):
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                parameter_name="prompt",
                node_name=source_node,
                value="Close up photorealistic photo of one person's face and shoulders, looking straight into the camera, evenly lit, plain gray background",
                initial_setup=True,
                is_output=False,
            )
        )
    audio_node = GriptapeNodes.handle_request(
        CreateNodeRequest(
    ...
    with GriptapeNodes.ContextManager().node(audio_node):
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                parameter_name="text",
                node_name=audio_node,
                value="Testing a longer line of speech so the generated clip runs past two seconds.",
                initial_setup=True,
                is_output=False,
            )
        )
    gen_node = GriptapeNodes.handle_request(
        CreateNodeRequest(
            ...
            specific_library_name="Griptape Nodes Library",
            node_name="OmnihumanVideoGeneration",
            metadata={},
            initial_setup=True,
        )
    ).node_name
    ...
    GriptapeNodes.handle_request(
        CreateConnectionRequest(
            source_node_name=audio_node,
            source_parameter_name="audio_url",
            target_node_name=gen_node,
            target_parameter_name="audio_url",
            initial_setup=True,
        )
    )
    with GriptapeNodes.ContextManager().node(gen_node):
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                parameter_name="prompt",
                node_name=gen_node,
                value="The person speaks to the camera",
                initial_setup=True,
                is_output=False,
            )
        )
