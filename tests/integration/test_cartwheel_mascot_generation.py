...
# dependencies = []
# [tool.griptape-nodes]
# name = "test_cartwheel_mascot_generation"
...
# node_types_used = [["Griptape Nodes Testing Library", "AssertString"], ["Griptape Nodes Library", "EndFlow"], ["Griptape Nodes Library", "CartwheelMascotGeneration"], ["Griptape Nodes Library", "StartFlow"], ["Griptape Nodes Library", "ToText"]]
...
with GriptapeNodes.ContextManager().flow(flow_name):
    gen_node = GriptapeNodes.handle_request(
        CreateNodeRequest(
            node_type="CartwheelMascotGeneration",
            specific_library_name="Griptape Nodes Library",
            node_name="CartwheelMascotGeneration",
            metadata={},
            resolution="resolved",
            initial_setup=True,
        )
    ...
    with GriptapeNodes.ContextManager().node(start_node):
        ...
        GriptapeNodes.handle_request(
            AddParameterToNodeRequest(
                parameter_name="input_video_media_id",
                default_value="",
                tooltip="Input Video Media ID",
                type="str",
                input_types=["any"],
                output_type="str",
                ui_options={"display_name": "Input Video Media ID"},
                parent_container_name="",
                initial_setup=True,
            )
        )
        GriptapeNodes.handle_request(
            AddParameterToNodeRequest(
                parameter_name="character_reference_media_ids",
                default_value=[],
                tooltip="Character Reference Media IDs",
                type="list",
                input_types=["list"],
                output_type="list",
                ui_options={"display_name": "Character Reference Media IDs"},
                parent_container_name="",
                initial_setup=True,
            )
        )
    ...
    GriptapeNodes.handle_request(
        CreateConnectionRequest(
            source_node_name=start_node,
            source_parameter_name="input_video_media_id",
            target_node_name=gen_node,
            target_parameter_name="input_video_media_id",
            initial_setup=True,
        )
    )
    GriptapeNodes.handle_request(
        CreateConnectionRequest(
            source_node_name=start_node,
            source_parameter_name="character_reference_media_ids",
            target_node_name=gen_node,
            target_parameter_name="character_reference_media_ids",
            initial_setup=True,
        )
    )
    GriptapeNodes.handle_request(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-video-media-id", default=None)
    parser.add_argument("--character-reference-media-ids", nargs="+", default=None)
    ...
    if args.json_input is None:
        ...
        if args.input_video_media_id is not None:
            flow_input["Start Flow"]["input_video_media_id"] = args.input_video_media_id
        if args.character_reference_media_ids is not None:
            flow_input["Start Flow"]["character_reference_media_ids"] = args.character_reference_media_ids
