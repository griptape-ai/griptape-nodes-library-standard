...
# node_types_used = [["Griptape Nodes Testing Library", "AssertFileExists"], ["Griptape Nodes Library", "CreateColorBars"], ["Griptape Nodes Library", "EndFlow"], ["Griptape Nodes Library", "QwenImageEdit"], ["Griptape Nodes Library", "ToText"]]
...
with GriptapeNodes.ContextManager().flow(flow_name):
    ...
    with GriptapeNodes.ContextManager().node(gen_node):
        GriptapeNodes.handle_request(
            AddParameterToNodeRequest(
                parameter_name="images_ParameterListUniqueParamID_00000000000000000000000000000001",
                default_value=[],
                tooltip="List of 1-6 images to edit",
                type="ImageArtifact",
                input_types=["ImageArtifact", "ImageUrlArtifact", "str"],
                output_type="ImageArtifact",
                ui_options={},
                parent_container_name="images",
                initial_setup=True,
            )
        )
    GriptapeNodes.handle_request(
        CreateConnectionRequest(
            ...
            target_parameter_name="images_ParameterListUniqueParamID_00000000000000000000000000000001",
            initial_setup=True,
        )
    )
    with GriptapeNodes.ContextManager().node(gen_node):
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                parameter_name="editing_instruction",
                node_name=gen_node,
                value="Make it blue",
                initial_setup=True,
                is_output=False,
