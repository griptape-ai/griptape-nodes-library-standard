...
# dependencies = []
# [tool.griptape-nodes]
# name = "test_openai_image_generation_wide"
...
Exercises the GPT Image 2 size choices added for issue #202: confirms the engine
accepts a non-square aspect ratio that the original 3-option dropdown did not
expose, and that the proxy returns a usable image at that size.
"""
...
with GriptapeNodes.ContextManager().flow(flow_name):
    ...
    with GriptapeNodes.ContextManager().node(gen_node):
        ...
        GriptapeNodes.handle_request(SetParameterValueRequest(parameter_name="size", value="1792x1024"))
